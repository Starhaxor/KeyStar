...
const deviceColumns = `
	id::text, application_id::text, user_id::text, license_id::text, tpm_public_key, tpm_public_key_sha256,
...
func (locked *LockedChallenge) ListDevices(ctx context.Context) ([]domain.Device, error) {
	rows, err := locked.tx.Query(ctx, `
		...
		from devices
		where license_id = $1 and application_id = $2::uuid and user_id = $3
...
func (locked *LockedChallenge) CreateDevice(ctx context.Context, input domain.NewDevice) (*domain.Device, error) {
	device, err := scanDevice(locked.tx.QueryRow(ctx, `
		insert into devices (
			application_id, user_id, license_id, tpm_public_key, tpm_public_key_sha256,
			...
			nullif($8, ''), nullif($9, ''), $10, $11, $12, $12, $12
		)
		returning `+deviceColumns,
		locked.applicationID, locked.userID, locked.licenseID, input.TPMPublicKey, input.TPMPublicKeySHA256,
...
func (locked *LockedChallenge) UpdateDevice(ctx context.Context, input domain.UpdateDevice) error {
	tag, err := locked.tx.Exec(ctx, `
		...
		set smbios_uuid_hmac = nullif($2, ''),
			...
			updated_at = $8,
			last_seen_at = $8
		where id = $1 and application_id = $9::uuid and license_id = $10 and user_id = $11 and status = 'active'`,
		...
		input.SeenAt, locked.applicationID, locked.licenseID, locked.userID)
	if err != nil {
...
func (locked *LockedChallenge) MarkSessionVerified(ctx context.Context, now time.Time) error {
	tag, err := locked.tx.Exec(ctx, `
		...
		where id = $1 and application_id = $3::uuid and status = 'pending'`, locked.sessionID, now, locked.applicationID)
	if err != nil {
...
func scanDevice(row pgx.Row) (*domain.Device, error) {
	...
	err := row.Scan(
		&device.ID, &device.ApplicationID, &device.UserID, &device.LicenseID, &device.TPMPublicKey, &device.TPMPublicKeySHA256,
