...
export default function ProductLifecycleSection({ product, plans, canWrite, onRefresh, onAddPlan, historical = false }: { product: Product; plans: Plan[]; canWrite: boolean; onRefresh: () => Promise<void>; onAddPlan: (product: Product) => void; historical?: boolean }) {
  ...
  async function copyProductId() {
    try {
      ...
      // Clipboard may be unavailable; the ID stays visible for manual copy.
    }
  }
  ...
  return <section className="border-b border-gray-200 p-5 last:border-0 dark:border-gray-800">
    <div className="flex flex-wrap items-start justify-between gap-3"><div><div className="flex items-center gap-2"><h2 className="font-semibold">{product.name}</h2><StatusBadge status={product.status} /></div><p className="text-xs text-gray-500">{product.slug}</p><p className="mt-1 flex items-center gap-2 font-mono text-xs text-gray-500">ID: {product.id}<button type="button" className="rounded border border-gray-300 px-1.5 py-0.5 text-[11px] font-medium text-gray-600 hover:bg-gray-100 dark:border-gray-700 dark:text-gray-300 dark:hover:bg-white/10" onClick={copyProductId}>{copiedId ? "Copied" : "Copy"}</button></p>{historical && <p className="mt-1 text-xs text-gray-500">Historical catalog relationship</p>}</div>{canWrite && !readOnly && <div className="flex flex-wrap gap-2"><Button type="button" size="sm" variant="outline" onClick={() => onAddPlan(product)}>Add plan</Button><Button type="button" size="sm" variant="outline" onClick={openProductEdit}>Edit product</Button><Button type="button" size="sm" variant="outline" onClick={() => setArchiveTarget({ kind: "product", product })}>Archive product</Button></div>}</div>
    <div className="mt-4 grid gap-3 md:grid-cols-3">{plans.map((plan) => { const planArchived = plan.status === "archived"; return <div key={plan.id} className="rounded-xl border border-gray-200 p-3 text-sm dark:border-gray-700"><div className="flex items-start justify-between gap-2"><strong>{plan.name}</strong><StatusBadge status={plan.status} /></div><p className="mt-1 text-gray-500">{plan.code} · Level {plan.level} · {plan.max_devices} devices</p><p className="mt-1 text-gray-500">Default duration: {formatDuration(plan.default_duration_seconds) ?? "not set"}</p>{canWrite && !readOnly && !planArchived && <div className="mt-3 flex gap-2"><Button type="button" size="sm" variant="outline" onClick={() => openPlanEdit(plan)}>Edit plan</Button><Button type="button" size="sm" variant="outline" onClick={() => setArchiveTarget({ kind: "plan", plan })}>Archive plan</Button></div>}</div>; })}{plans.length === 0 && <p className="text-sm text-gray-500">No plans yet.</p>}</div>
