...
export default function ProductLifecycleSection({ product, plans, canWrite, onRefresh, onAddPlan, historical = false }: { product: Product; plans: Plan[]; canWrite: boolean; onRefresh: () => Promise<void>; onAddPlan: (product: Product) => void; historical?: boolean }) {
  ...
  return <section className="border-b border-gray-200 p-5 last:border-0 dark:border-gray-800">
    <div className="flex flex-wrap items-start justify-between gap-3"><div><div className="flex items-center gap-2"><h2 className="font-semibold">{product.name}</h2><StatusBadge status={product.status} /></div><p className="text-xs text-gray-500">{product.slug}</p>{historical && <p className="mt-1 text-xs text-gray-500">Historical catalog relationship</p>}</div>{canWrite && !readOnly && <div className="flex flex-wrap gap-2"><Button type="button" size="sm" variant="outline" onClick={() => onAddPlan(product)}>Add plan</Button><Button type="button" size="sm" variant="outline" onClick={openProductEdit}>Edit product</Button><Button type="button" size="sm" variant="outline" onClick={() => setArchiveTarget({ kind: "product", product })}>Archive product</Button></div>}</div>
